...
| `favicon` | `permissions` | Displays website favicons in the side panel tab list, search drawer, and bookmarks view to provide instant visual identification of saved links and active tabs. It is also what gives an automatic tab group the colour of the site it holds: the icon Chrome already has is read locally and its dominant colour mapped to one of Chrome's nine group colours. |
