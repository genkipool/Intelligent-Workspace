...
<section id="persistent-conversation-controls" class="hidden-groups-container hidden">
    ...
    <button
        ...
        onclick={handleViewConversations}>{currentConversationTitle || $t('selectConversationPlaceholder')}</button
    >
    <button
        type="button"
        id="cycle-next-conversation-btn"
        title={$tt('nextSavedConversation')}
