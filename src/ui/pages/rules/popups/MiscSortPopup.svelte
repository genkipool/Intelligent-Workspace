...
{#if show}
    <div class="misc-sort-popup open" style="left: {position.x}px; top: {position.y}px;" bind:this={popupEl}>
        ...
        <div class="misc-sort-options-container">
            <button class="option-button" data-value="start" onclick={() => select('start')}
                >{$t('miscSortStart')}</button
            >
            <button class="option-button" data-value="end" onclick={() => select('end')}>{$t('miscSortEnd')}</button>
            <button class="option-button" data-value="alpha" onclick={() => select('alpha')}
