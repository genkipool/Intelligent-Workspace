...
var prettierPluginBabel = require('prettier/plugins/babel');
    ...
    switch (node.type) {
        ...
        case 'AwaitBlock': {
            ...
            if (!hasPendingBlock && hasThenBlock) {
                ...
                if (hasThenBlock) {
                    block.push(group(['{:then', expandNode(node.value, options.originalText), '}']), printBlockFragment(path, print, options, 'then'));
                }
            }
            if ((hasPendingBlock || hasThenBlock) && hasCatchBlock) {
                block.push(group(['{:catch', expandNode(node.error, options.originalText), '}']), printBlockFragment(path, print, options, 'catch'));
            }
            block.push('{/await}');
            return group(block);
        }
...
function printSvelteOptions(path, parserOptions, print, rootOptions, leadingCommentDoc) {
    ...
    let doc = group([
        [
            ...
            optsNode.name,
            indent(group([
                ...path.call((optionsPath) => optionsPath.map(printWithPrependedAttributeLine(optsNode, parserOptions, print), 'attributes'), 'options'),
                bracketSameLine ? '' : dedent(line),
            ])),
            ...[bracketSameLine ? ' ' : '', '/>'],
        ],
...
function printTopLevelParts(n, options, path, print) {
    if (options.svelteSortOrder === 'none') {
        if (n.options) {
            const nodes = n.fragment.nodes;
            const has_options_node = nodes.some((node) => node.type === 'Options');
            if (!has_options_node) {
                nodes.push({
                    type: 'Options',
                    name: 'svelte:options',
    ...
    if (alternate.type === 'Fragment') {
        if (alternate.nodes.length === 1 &&
            ...
            if (alternate.nodes[0].alternate) {
                def.push(path.call((if_path) => printIfBlockAlternate(if_path, print, options), 'alternate', 'nodes', 0));
            }
            return def;
        }
...
function printChildren(path, print, options) {
    ...
    const original_children = (_a = current_value.nodes) !== null && _a !== void 0 ? _a : [];
    const prepared_children = prepareChildren(original_children);
    ...
    const forceBreakContent = prepared_children.length > 1 &&
...
function _expandNode(node, original, parent) {
    ...
    switch (node.type) {
        ...
        case 'ArrayPattern':
            return (' [' +
                node.elements
                    ...
                    .map((el) => (el === null ? ' ' : _expandNode(el, original)))
                    .join(',')
                    .slice(1) +
                ']');
        ...
        case 'ObjectExpression':
            return (' {' +
                node.properties.map((p) => _expandNode(p, original, node)).join(',') +
                ' }');
        case 'ObjectPattern':
            return (' {' + node.properties.map((p) => _expandNode(p, original)).join(',') + ' }');
        case 'Property': {
            let computedKeyInner = '';
            if (node.computed) {
                computedKeyInner =
                    typeof ((_a = node.key) === null || _a === void 0 ? void 0 : _a.start) === 'number' && typeof ((_b = node.key) === null || _b === void 0 ? void 0 : _b.end) === 'number'
...
function embed(path, _options) {
    ...
    const parent = path.getParentNode();
    const printJsExpression = () => parent.expression ? printJS$1(parent, 'expression', {}) : undefined;
    ...
    switch (parent.type) {
        ...
        case 'ExpressionTag':
            printJS$1(parent, 'expression', {
                forceSingleQuote: isInsideQuotedAttribute(path),
            });
            break;
        case 'HtmlTag':
            printJS$1(parent, 'expression', {});
...
function embedTag(tag, text, path, formatBodyContent, print, isTopLevel, options) {
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const previousComments = node.type === 'Script' || node.type === 'StyleSheet'
            ? ((_a = node.comments) !== null && _a !== void 0 ? _a : [])
            : [getLeadingComment(path)]
                .filter(Boolean)
                .map((comment) => ({ comment: comment, emptyLineAfter: false }));
        const canFormat = isNodeSupportedLanguage(node) &&
            !isIgnoreDirective((_b = previousComments[previousComments.length - 1]) === null || _b === void 0 ? void 0 : _b.comment) &&
            (tag !== 'template' ||
                options.plugins.some((plugin) => typeof plugin !== 'string' &&
...
function walkAndAttach(node, commentsByStart, original_text) {
    ...
    if ('attributes' in node && Array.isArray(node.attributes) && node.attributes.length > 0) {
        ...
        const opening_tag_end = original_text && typeof node.end === 'number'
    ...
    if (node.type === 'IfBlock') {
        ...
        if (node.pending)
            walkAndAttach(node.pending, commentsByStart, original_text);
        if (node.then)
            walkAndAttach(node.then, commentsByStart, original_text);
        if (node.catch)
            walkAndAttach(node.catch, commentsByStart, original_text);
    }
    else {
...
function attachCommentsInRange(rangeStart, rangeEnd, precedingAttr, followingAttr, commentsByStart) {
    for (const [start, comment] of commentsByStart) {
        if (start >= rangeStart && comment.end <= rangeEnd) {
            if (followingAttr) {
                prettier.util.addLeadingComment(followingAttr, comment);
            }
            else if (precedingAttr) {
                prettier.util.addTrailingComment(precedingAttr, comment);
            }
